...
bool second_chance(std::vector<int> vector, int pos) {
  bool is_safe = true;
  vector.erase(vector.begin() + pos);

  check_safety(vector, is_safe);
  return is_safe;
}
...
int main() {
  ...
  if (file.is_open()) {
    ...
    while (getline(file, line)) {
      ...
      if (!is_safe) {
        for (int i = 0; i < (int)report.size() && !is_safe; ++i) {
          is_safe = second_chance(report, i);
        }
      }
